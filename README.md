...
> *A real-time AI co-pilot for 911 dispatchers. Every second costs a life — we gave them an AI that thinks in milliseconds.*
...
In the next 90 seconds of a 911 call, a single dispatcher must:
...
- Search unit availability, calculate closest unit with live traffic
- Cross-reference hospital specialties and bed availability
- Determine if hazmat, fire, or additional backup is needed
...
**ARIA brings a layer of verification and grounding** to every decision. Every recommendation is grounded in authoritative source documents — FEMA ERG, AHA protocols, MPDS triage guidelines. The system catches errors that happen when human judgment operates under impossible conditions.
...
Current systems are built for discrete packets of information. **ARIA's Stream Processor reacts to every word the moment it is spoken** — domain watchers fire agents in real time, the recommendation card updates as the situation evolves. Human judgment combined with machine speed.
...
| **3.5. Verifier** | Claude Haiku 3.5 | Parallel verifier (never blocking) — resolves ambiguities, enriches context mid-run |
...
| Service | Purpose |
|---------|---------|
| Amazon Bedrock (Claude Haiku 3.5) | Fast real-time verifier — sub-second latency |
| Amazon Bedrock (Claude Sonnet 4) | Coordinator synthesis + specialist agent reasoning |
...
| Amazon API Gateway (REST + WebSocket) | REST endpoints + live dashboard WebSocket spine |
...
│   ├── ARIA_TechStack.md     # Technology choices & cost profile
